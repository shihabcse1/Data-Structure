...
void DoublyLinkedList :: prepend(int item){
...
void DoublyLinkedList :: insert(int item, int key){
    Node *newNode = new Node();
    newNode->data = item;
    newNode->next = NULL;
    if(head == NULL){
...
void DoublyLinkedList :: search(int item){
    Node *temp = head;
    while(temp != NULL){
        if(temp->data == item){
            break;
...
int main(){
    ...
    do{
        ...
        case 5:
            cout<<"After which nodes you want to insert(Enter NewNode's value First, Then Enter key) ? "<<endl;
            cin>>value>>keyValue;
